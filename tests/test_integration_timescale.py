...
async def test_outbox_leases_retry_and_complete_without_two_workers_owning_a_row() -> None:
    ...
    try:
        ...
        first = await repository.claim_outbox("worker-1", producer="integration-producer", limit=1)
        assert len(first) == 1
        assert first[0].message_id == message_id
        assert first[0].publish_attempts == 1
        assert await repository.claim_outbox("worker-2", producer="integration-producer", limit=1) == []
        ...
        second = await repository.claim_outbox("worker-2", producer="integration-producer", limit=1)
        assert len(second) == 1
